...
const ServiceCard = ({ service, isLoading = false }) => {
    ...
    return (
        <Motion.div
            whileHover={{ y: -8 }}
            className="bg-base-100 rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 h-full flex flex-col"
        >
            {/* Image Container */}
            ...
            <div className="p-4 flex flex-col flex-grow">
                ...
                <div className="space-y-2 mb-4 text-sm">
                    ...
                    <div className="flex items-center gap-2 text-base-content/60">
                        ...
                        {service?.city}, {service?.district}, {service?.zip}
                    </div>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="flex items-center gap-1">
                                {[...Array(5)].map((_, i) => (
                                    <FaStar
                                        key={i}
                                        className={`text-sm ${i < Math.floor(service?.averageRating)
                                            ? 'text-red-500'
                                            : 'text-neutral-300'
                                            }`}
                                    />
                            ...
                            <span className="text-base-content/60">({service?.reviews.length || 0})</span>
                        </div>
                    )}
                </div>
                ...
                <Link
                    ...
                    className="btn btn-sm text-red-500 border-red-500 hover:bg-red-500 hover:text-white w-full hover:btn-error"
                >
                    View Details
                </Link>
            </div>
